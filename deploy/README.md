...
The GPU image sets **`TALKIE_QUANTIZATION=nf4`**: the loader uses **meta** tensors plus **block streaming** (one transformer block resident on CPU at a time) so peak RAM stays near **one block + embeddings** instead of the full 13B matrix on CPU, then moves **NF4** linears to the GPU. The **embedding** and **`lm_head`** stay **bf16** on GPU (~1.3 GiB combined for a 64k vocab). **Quality** may be slightly lower than full bf16; set **`TALKIE_QUANTIZATION=none`** on a larger GPU if you need the original path.
